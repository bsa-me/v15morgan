...
class ResPartner(models.Model):
    ...
    def migrate_companies(self):
        ...
        currentOrder = self.env['ir.config_parameter'].browse(68).value
        hasData = True

        while(hasData):
            response = requests.get(mrm_url + 'crm/companies?id__gt=' + str(currentOrder) + '&_order=id&_limit=100',
                ...
                if 'email' in details_response:
                    if details_response['email']:
                        if len(details_response['email']) > 0:
                            contact = models.execute_kw(mip_db, uid, mip_password,'res.partner', 'search',[[['email', '=', details_response['email']]]])
                            if not contact:
