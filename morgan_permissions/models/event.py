...
class Event(models.Model):
	...
	is_super_user = fields.Boolean(compute="_compute_user_groups")
	...
	def action_archive(self):
		...
		return super(Event, self).action_archive()

	def _compute_user_groups(self):
		for record in self:
			record['is_super_user'] = False
			...
			if self.env.user.has_group('morgan_permissions.super_user_group'):
				record['is_super_user'] = True
