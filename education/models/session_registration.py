...
class SessionRegistration(models.Model):
    ...
    def _check_seats_limit(self):
        for registration in self:
            if registration.session_id.event_id.seats_availability == 'limited' and registration.session_id.event_id.seats_max and registration.session_id.event_id.seats_available < (1 if registration.state == 'draft' else 0):
    ...
    def get_date_range_str(self):
        ...
        if diff.days <= 0:
            ...
            return _('on ') + format_datetime(self.env, self.session_id.date, tz=self.session_id.event_id.date_tz, dt_format='medium')
