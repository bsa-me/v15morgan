...
class SaleCouponProgram(models.Model):
	...
	def _check_promo_code(self, order, coupon_code):
		...
		elif self.promo_applicability == 'on_current_order' and self.reward_type == 'product' and order._is_reward_in_order_lines(self):
	...
	def _filter_not_ordered_reward_programs(self, order):
		...
		for program in self:
			if program.reward_type == 'product' and order.order_line.filtered(lambda line: line.product_id == program.reward_product_id):
				continue
			elif program.reward_type == 'discount' and program.discount_apply_on == 'specific_products' and not order.order_line.filtered(lambda line: line.product_id in program.discount_specific_product_ids):
				continue

			elif program.reward_type == 'discount' and program.condition == 'and' and program.discount_apply_on == 'specific_events' and not set(program.course_ids.ids).issubset(set(order.order_line.mapped('event_id').mapped('course_id').ids)):
				continue

			elif program.reward_type == 'discount' and program.condition == 'any' and program.discount_apply_on == 'specific_events' and True not in ([a in order.order_line.mapped('event_id').mapped('course_id').ids for a in program.course_ids.ids]):
				continue
