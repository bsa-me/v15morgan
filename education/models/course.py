...
class Course(models.Model):
    ...
    clean_subject_area = fields.Char(string='Clean Subject Area',compute="_compute_subject_area")
            ...
            if companyPrice.company_id.region_ids:
                for region in companyPrice.company_id.region_ids:
                    ...
                    for ticketPrice in companyPrice.ticket_ids:
                        mappedItems = []
                        company_price = self.env.ref('base.USD')._convert(ticketPrice.price,region.main_pricelist_id.currency_id,companyPrice.company_id,date)
                ...
                for ticketPrice in companyPrice.ticket_ids:
                    mappedItems = []
                    company_price = self.env.ref('base.USD')._convert(ticketPrice.price,companyPrice.company_id.currency_id,companyPrice.company_id,date)
                    for product in ticketPrice.mapped_item_ids:
                        mappedItems.append(product.id)
    ...
    def action_view_instructors(self):
        employees = self.env['hr.contract'].search([('course_ids', 'in', self.id),('state','=','open')]).mapped('employee_id')
        return {
            "name": _("Instructors"),
    ...
    def _compute_count_instructors(self):
        for record in self:
            employees = []
            contracts = self.env['hr.contract'].search([('course_ids', 'in', record.id),('state','=','open')])
            for contract in contracts:
                if contract.employee_id.id not in employees:
                    employees.append(contract.employee_id.id)
            record['instructor_count'] = len(employees)
    ...
    def _compute_subject_area(self):
        for record in self:
            record['clean_subject_area'] = ''
            if record.subject_area:
                clean = re.compile('<.*?>')
                subject_area = re.sub(clean, '', record.subject_area)
                subject_area = subject_area.replace("&nbsp;"," ")
                record['clean_subject_area'] = subject_area
