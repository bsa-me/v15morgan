...
class ResCompany(models.Model):
    ...
    report_footer = fields.Html('Report Footer', translate=True)
    ...
    bank_account_ids = fields.One2many('account.journal','company_id', domain=lambda self:[('type','=','bank'),('name','!=','Instructor Payments')])
    ...
    document_sequence = fields.Boolean('Followed by parent company',help='if checked, the sequence of the company related document will follow the parent company')
    ...
    invoice_name = fields.Char('Invoice name', translate=True)
    credit_note_name = fields.Char('Credit note name',translate=True)
    receipt_name = fields.Char('Receipt Name',translate=True)
    refund_name = fields.Char('Refund Name',translate=True)
    quotation_name = fields.Char('Quotation Name',translate=True)
    soa_name = fields.Char('SOA Name',translate=True)
    invoice_issuance = fields.Boolean('Invoice from parent company',help='if checked, the billing company would be the parent company')
