...
class ResPartner(models.Model):
    ...
    instructor_company = fields.Char('Instructor Region')
    ...
    instructor_attachment_ids = fields.One2many('documents.document', 'partner_id',
        string='Documents')
    ...
    date_activation = fields.Date('Activation Date',compute="compute_activation",store=False)

    name = fields.Char(index=True,readonly=False,required=True)
    decision_maker_id = fields.Many2one('decision.maker',string='Decision Maker')

    mrm_id = fields.Integer('MRM ID')
    mrm_bank_details = fields.Text('MRM Bank Details')

    parent_mrm_id = fields.Integer()
    not_same_as_region = fields.Boolean(string='Different Location Address')
    registration_ids = fields.One2many('event.registration','partner_id',string='Registrations')
    opt_out = fields.Boolean()
    left_company = fields.Boolean()
    is_staff = fields.Boolean()
    organisme_collecteur = fields.Char('Organisme Collecteur')
    fonction = fields.Char('Fonction')
    address_unit = fields.Char('Address Unit')
    dont_mail = fields.Boolean()
    no_more_adds = fields.Selection([('0','0'),('1','1')])
    primary_contact_name = fields.Char('Primary Contact Name')
    account_size = fields.Selection([('small', 'Small'),('medium', 'Medium'),('big','Big')],string='Account Size')
    address_region = fields.Char('Address Region')
    mrm_create_date = fields.Date('MRM Create Date')
    parent_account_id = fields.Integer('Parent Account ID')
    balance = fields.Monetary(currency_field='balance_currency')
    balance_currency = fields.Many2one('res.currency', default=_default_currency)
    open_fp_amount = fields.Monetary(currency_field='balance_currency',string='Fps Amount Due')
    ...
    _sql_constraints = [
    ('name_uniq', 'Check(1=1)', "Contact name already exists !"),
    ]
    ...
    _sql_constraints = [
    ('email_uniq', 'unique(email)', "Contact email already exists !"),
    ]

    def compute_attendance(self):
        for record in self:
            record.avg_attendance = 0
            attendances = self.env['attendance'].search([('partner_id','=',record.id)])
            record.attendance_count = len(attendances)

            attended = self.env['attendance'].search([('partner_id','=',record.id),('attended','=',1)])
    ...
    def show_related_attendaces(self):
        attendances = self.env['attendance'].search([('partner_id','=',self.id)])
        if attendances:
            res = {
    ...
    @api.onchange('not_same_as_region','company_id')
    def _onchange_not_same_as_region(self):
        if(self.not_same_as_region):
            return {
            'value': {'street': False, 'street2': False, 'country_id': False, 'state_id': False, 'city': False, 'zip': False},
            }

        elif(not self.not_same_as_region and self.company_id):
            contact = self.company_id
            street=contact.street
            stree2=contact.street2
            city=contact.city
            country=contact.country_id.id if contact.country_id else False
    ...
    def compute_activation(self):
        for record in self:
            status = False
            invoices = self.env['account.move'].search([('partner_id','=',record.id),('state','=','posted'),('move_type','=','out_invoice'),('amount_residual','>',0)])
            if(not invoices):
                status = 'pending'

            else:
                last_invoice = self.env['account.move'].search([('partner_id','=',record.id),('state','=','posted'),('move_type','=','out_invoice'),('amount_residual','>',0)],order="invoice_date desc",limit=1)
    ...
    def create(self, vals):
        ...
        if record.account_type == 'b2b' or record.account_type == 'b2u':
            ...
            if record.parent_id:
                record['company_legal_name'] = record.parent_id.name
        return record
    ...
    """def write(self, vals):
        for record in self:
            ...
            if phone:
                phone = "'" + phone + "'"
                self.env.cr.execute("update crm_lead set phone = " + phone + " where partner_id = " + str(record.id))

            
            if mobile:
                mobile = "'" + mobile + "'"
                self.env.cr.execute("update crm_lead set mobile = " + mobile + " where partner_id = " + str(record.id))

            if email:
                email = "'" + email + "'"
                self.env.cr.execute("update crm_lead set email_from = " + email + " where partner_id = " + str(record.id))
    ...
    def check_account_type_b2b(self):
        if self.account_type == 'b2c' and self.child_ids:
            raise ValidationError("You cannot change the account type to b2c for this customer as he already has contacts!")"""
