...
class ProductRegistration(models.Model):
    ...
    attempt_ids = fields.One2many('attempt','product_registration_id',string="Attempts", domain=lambda self:[('type','=','attempt')])
    attempt_exam_ids = fields.One2many('attempt','product_registration_id',string="Attempts", domain=lambda self:[('type','=','examhomework')])
