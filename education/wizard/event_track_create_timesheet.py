...
class EventTrackCreateTimesheet(models.TransientModel):
    ...
    def save_timesheet(self):
        session = self.env['event.track'].browse(self.session_id.id)
        start_time = session.timesheet_timer_start
        session.write({
            'start_date': self.check_in,
            'end_date': self.check_out,
            'stage_id': self.env.ref('website_event_track.event_track_stage1').id
            })
